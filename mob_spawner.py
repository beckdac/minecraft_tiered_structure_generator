...
E.g. usage:
./mob_spawner.py -x 82 -z 358 -y 63 -b minecraft:cobblestone
...
def fall_trap_level(x_center, y, z_center, block_id):
    ...
    set_block(x, y + y_current, z + 2, block_id)
...
def mob_spawner(x_center, y_bottom, z_center, block_id):
    ...
    z = z_center
    for y in range(2, 4):
        fall_trap_level(x_center, y + y_bottom, z_center, "minecraft:glass")
    for y in range(4, chamber_height):
        fall_trap_level(x_center, y + y_bottom, z_center, block_id)
    # spawning chamber
